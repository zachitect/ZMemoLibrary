...
# LLM Instruction - Update Existing Knowledge File
...
1. The existing knowledge file.
2. The new update content.

If the existing file or update content is missing, ask for it instead of guessing.

## Update Rules

- Preserve the existing `ID` exactly.
- Preserve `CREATED` unless it is clearly wrong.
- Change `UPDATED` only when the body receives new durable content.
- Do not change `UPDATED` for header-only, rename-only, or connection-only changes.
- Update `TITLE`, `SUMMARY`, and `KEYWORDS` only when useful.
- Merge new durable information into the body.
- Remove or replace outdated information when the update supersedes it.
- Do not append a raw session log.
- Do not invent facts.
- If multiple files are provided, update the target file and find connections using visible GUIDs.
